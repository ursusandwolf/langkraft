...
### 1. Состояние гонки (Race Condition) в `SrsTrainingViewModel`

**Файл:** `SrsTrainingViewModel.kt`

**Проблема:** В методе `loadQueue()` ViewModel подписывается на реактивный `Flow` базы данных (`vocabularyRepository.getWordsToReview().collect`). При вызове `submitResult` происходит следующее:
1. Вызывается `vocabularyRepository.saveWord(updatedWord)`. Это действие триггерит SQLDelight, и Flow **снова** эмиттит список слов.
2. Одновременно с этим (в той же корутине), локальный стейт мутируется вручную: `val nextQueue = it.queue.drop(1)`.

Поскольку Flow обновляется асинхронно, UI может мигать: сначала очередь уменьшится на 1 через локальный `.drop(1)`, а затем прилетит апдейт от БД, который может перезаписать эту очередь. Это классический антипаттерн MVI (две точки правды).

**Как исправить:** 
Убрать ручное манипулирование `queue` (удалить `drop(1)`). Поскольку SQLDelight Flow реактивный, сохранение слова автоматически обновит запрос `getWordsToReview()`, убрав изученное слово из выборки. Нужно доверять единому источнику правды (Single Source of Truth).
